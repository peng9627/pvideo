        ...
        if data is not None:
            ...
            try:
                ...
                if "span" in data:
                    ...
                    if len(move_span) > 0:
                        wheres.append(
                            "span LIKE '%" + move_span + "%' OR title LIKE '%" + move_span + "%' OR child_type LIKE '%" + move_span + "%' ")
                if "type" in data:
                    move_type = data["type"]
                    if len(move_type) < 2:
    ...
    if key is not None:
        ...
        if data is not None:
            ...
            try:
                ...
                connection = mysql_connection.get_conn()
                videos = data_movie.search(connection, pymysql.converters.escape_string(content), page, pageSize)
                result = '{"state":0, "data":[%s]}' % ",".join(videos)
            except:
                logger.exception(traceback.format_exc())
            finally:
            ...
            try:
                ...
                if movie is not None:
                    ...
                    if 0 == code:
                        ...
                        if data_vip.is_vip(connection, account_id):
                            is_vip = True
                    result = '{"state":0, "data":%s,"content":"%s","is_vip":%s}' % (
                        json.dumps(movie.__dict__), content, 'true' if is_vip else 'false')
            except:
                logger.exception(traceback.format_exc())
            finally:
                if connection is not None:
